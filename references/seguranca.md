...
`public.reordenar_opcionais_da_categoria(p_loja_id uuid, p_categoria_id uuid, p_ids uuid[])` nasceu na issue 208 (migrations `20260917120000_ordem_em_categoria_produto_opcionais.sql` + `20260917121000_rpc_reordenar_opcionais_da_categoria.sql`) como segunda instância do mesmo padrão — `SECURITY INVOKER`, mesma base. **Isso mudou na issue 215** (`20260918121000_rpc_reordenar_opcionais_da_categoria_definer.sql`): a função foi convertida para `SECURITY DEFINER` — ver subseção abaixo, que é hoje a descrição válida. **Diferença estrutural que sobrevive à conversão, não de estilo:** a permutação completa de `reordenar_categorias` é da **loja inteira**; a de `reordenar_opcionais_da_categoria` é do **par (loja, categoria de produto)** — o mesmo grupo de opcional pode estar associado a várias categorias de produto com posições diferentes em cada uma. Por isso a função recebe um segundo parâmetro de escopo, `p_categoria_id`, que **vem do payload do cliente** (os demais, `p_loja_id` incluído, vêm de `auth.uid()` no servidor, ou do `lojaId` da URL já validado na via admin). Três camadas cobrem esse parâmetro: a Server Action valida `categoria_id` como pertencente à loja com `categoriaProdutoPertenceALoja` **antes** de chamar a RPC; a RPC filtra o `UPDATE` por `and cpo.categoria_id = p_categoria_id`; e a trava T3 do corpo (ver abaixo) confere a coerência do par. Um id de outra categoria dentro de `p_ids`, ou um `p_categoria_id` de outra loja, derruba a transação inteira (permutação incompleta → `row_count` diverge → `raise exception`).

- **`REVOKE` nomeando `anon` é obrigatório em toda função nova, não basta revogar de `PUBLIC`.** O Postgres concede `EXECUTE` a `PUBLIC` por padrão em função nova, **e** o projeto tem `ALTER DEFAULT PRIVILEGES ... GRANT ALL ON ROUTINES TO anon, authenticated, service_role` (`20260614008500:31`) — ao contrário do que este texto afirmava até a auditoria da issue 250, que corrigiu a redação nas três migrations de RPC e aqui. A consequência é prática: `anon` recebe `EXECUTE` por **entrada própria na ACL**, não por `PUBLIC`, então `REVOKE ALL ... FROM PUBLIC` sozinho **não o barra**. Some `anon` ao revoke. Note também que `service_role` **não** é alcançado por esse revoke: a via de serviço só é barrada por predicado explícito no corpo da função (é o que a T2 de `aplicar_cardapio_em_categoria` faz, contra `auth.uid()`). Ver §19 sobre os default privileges de tabelas e sequences, que são outra história.
- **Validar cardinalidade de array vindo do cliente com `cardinality()`, nunca `array_length(arr, 1)`.** Achado BAIXA corrigido por `20260908130000_cardinality_reordenar_categorias.sql`: `array_length` conta só a primeira dimensão — um array multidimensional passa pela checagem `v_enviadas = v_na_loja` declarando menos elementos do que o `unnest` de fato entrega, corrompendo a `ordinality` usada para derivar `ordem`. `cardinality()` conta todos os elementos, em todas as dimensões. Vale para as três RPCs de ordem desta família.

### RPC de escrita em lote reusada por lojista E admin — `SECURITY DEFINER` com travas no corpo (issue 215, fecha o débito 211)

Duas RPCs desta família passam a servir **os dois chamadores** — o lojista autenticado e o hub admin sob `service_role` — com o mesmo corpo: `public.reordenar_opcionais_da_categoria` (convertida de `invoker` para `definer`) e `public.reordenar_itens_do_grupo_opcional(p_loja_id uuid, p_categoria_opcional_id uuid, p_ids uuid[])` (nova, `20260918120000_rpc_reordenar_itens_do_grupo_opcional.sql` — escreve a ordem dos ITENS dentro de UM grupo de opcional; escopo é o par (loja, grupo), inclusive itens `ativo = false`, já que o painel enxerga os inativos e a vitrine lê a mesma ordem ignorando-os).

**Por que a conversão era necessária, e por que ela ACRESCENTA uma trava em vez de afrouxar algo.** `service_role` tem o atributo `BYPASSRLS` — a RLS não é reavaliada para ele em nenhum dos dois modos, nem invoker nem definer. Sob `invoker`, a única autoridade da função era a RLS de escrita do lojista; para o chamador admin essa RLS já era pulada, então a via de serviço rodaria **sem nenhuma checagem de tenant** se algum dia chamasse essa RPC — é por isso que o hub admin nunca a chamou e gravava a ordem com N `update` sequenciais fora de transação (débito 211, fechado por esta issue). Sob `definer`, a autoridade some da RLS por construção (ela nunca é avaliada dentro de uma função `definer`, para nenhum chamador) e passa a ser **provada no corpo** por T2 (autoridade) e T3 (coerência do par): isto SOMA uma checagem à via de serviço, que antes não tinha nenhuma — não retira nada do caminho do lojista, que troca "RLS externa" por trava interna equivalente. **Não copie a frase invertida** do cabeçalho de `20260918121000_rpc_reordenar_opcionais_da_categoria_definer.sql` nem do commit `e3cafd6`, que descrevem a mudança como o escopo "deixando de ser garantido pelo banco" sem essa distinção — para o lojista a garantia é relocada; para o admin ela é criada.

**As sete travas do corpo** (numeração do comentário em `reordenar_itens_do_grupo_opcional`; `reordenar_opcionais_da_categoria` implementa os mesmos passos, na mesma ordem, mas só rotula T1–T3 no corpo):

| Trava | O quê | Por quê nesta ordem |
|---|---|---|
| T1 | `p_ids` não vazio (`cardinality()`, nunca `array_length`) | primeiro filtro, mais barato |
| T2 | **Autoridade** — dono da loja (`lojas.dono_id = auth.uid()`) OU via de serviço | substitui a RLS perdida com o `definer`; tem que vir antes de tudo que segue |
| T3 | **Coerência** do par (loja, grupo/categoria) — o registro-pai realmente pertence à `p_loja_id` | roda DEPOIS de T2 e ANTES da contagem: se rodasse depois, a contagem vira oráculo de existência de grupo/categoria em loja alheia |
| T4 | Permutação COMPLETA do par (`count(*)` bate com `cardinality(p_ids)`), sem filtrar por `ativo` | garante que a lista enviada cobre todo o conjunto — senão a normalização 0..n-1 deixa buraco |
| T5 | `UPDATE` escreve só `ordem`, derivada de `ordinality - 1`; nunca `preco`/`nome`/`ativo`/`loja_id` vindos do cliente | escopo explícito além de T2/T3 |
| T6 | `row_count` pós-`UPDATE` bate com o total enviado | id de outra loja/grupo, inexistente ou duplicado derruba a transação inteira |
| T7 | `REVOKE ALL FROM public, anon` + `GRANT EXECUTE TO authenticated, service_role` | função ganha `EXECUTE` de `PUBLIC` por padrão; sem o revoke, `anon` chamaria a RPC direto com a anon key do bundle público |

**Sinais que NÃO servem dentro de uma `SECURITY DEFINER`, e os dois que sobram.** `current_user` vale o DONO da função (não o chamador) e `row_security_active()` é sempre `false` — os dois sinais que o projeto usa em outros pontos do RLS não valem aqui. A autoridade fica com dois sinais em CONJUNÇÃO: `current_setting('role', true)` (role efetivo da sessão) e `auth.role()` (claim `role` do JWT, já verificado pelo PostgREST). `coalesce(auth.role(), '')` é obrigatório, não estético: `auth.role()` devolve `NULL` quando não há JWT, e `not (NULL or false)` avalia para `NULL`, que o plpgsql trata como `ELSE` — sem o `coalesce`, a trava T2 virava fail-**OPEN** por omissão. Foi um achado real (não teórico) da auditoria pós-215, corrigido por `20260918130000_rpc_ordem_t2_fail_closed.sql`. A mesma migration fixa `SET search_path = public, pg_temp`, com `pg_temp` explicitamente por último — todo nome no corpo já é schema-qualificado, então isso é defesa em profundidade, não correção de um vetor hoje ativo.

**Armadilha de harness que escondeu o fail-open.** `tests/helpers/pglite.ts` definia `auth.role()` como `coalesce(current_setting('request.jwt.claims', true)::json->>'role', 'anon')` — mais permissivo que o cloud, que devolve `NULL` sem JWT (conferido por `supabase db dump --schema auth`, não suposto). Com esse fallback, nenhuma das 21 asserções afetadas conseguia alcançar o `NULL` que dispara o bug. Corrigido na mesma issue para espelhar o cloud com fidelidade (`nullif(...)` antes do cast, sem fallback para `'anon'`). **Harness de teste que diverge da produção numa função de autorização é uma classe de bug, não um detalhe do helper** — trate divergência de harness em código de autorização com a mesma seriedade que uma RLS mal escrita.

**Regra para devs e agentes — duas variantes válidas do padrão, não uma substituição:**
- **`SECURITY INVOKER`** quando só o lojista autenticado escreve e a RLS dele é a autoridade única — nenhum chamador `service_role` de verdade (`reordenar_categorias` é o exemplo vivo). PoC diferencial: trocar para `DEFINER` **sem** travas equivalentes no corpo permitiria a um lojista reescrever dados de OUTRA loja, porque `p_loja_id` é escolhido pelo próprio chamador e `where loja_id = p_loja_id` sozinho não segura nada — quem segura, sob invoker, é a RLS de escrita avaliada no `UPDATE`.
- **`SECURITY DEFINER` + T1..T7 no corpo** quando a via admin (`service_role`) precisa do MESMO caminho que o lojista. Segue `SET search_path = public, pg_temp` + `REVOKE ALL FROM public, anon` + `GRANT EXECUTE TO authenticated, service_role` + `cardinality()` para array do cliente + a ordem de travas da tabela acima (autoridade e coerência ANTES da contagem, senão a contagem vira oráculo de existência em loja alheia) + `coalesce(auth.role(), '')` na conjunção de autoridade.
...
**Padrão admin (issues 083–102):** Server Actions sob `service_role` precedidas de `verificarAdminSaaS()` — auth.uid() validado contra `ADMIN_EMAIL` antes de elevar para service_role; escopo por tenant obrigatório (`lojaId` validado como UUID), garantido pelo wrapper `EscopoLoja` (ver subseção abaixo) em vez de `.eq("loja_id"/"id", lojaId)` manual. Helpers em `src/lib/actions/admin-loja.ts` (`validarLojaIdAdmin`, `prepararContextoAdmin`, `revalidarLojaAdmin`). Nota: RLS não é a defesa aqui (service_role a bypassa) — o gate é `verificarAdminSaaS()` + escopo. Contrasta com o painel do lojista, onde a defesa primária é RLS (`auth.uid() = dono_id`). Log de acesso admin: `registrarAcessoAdmin` (`admin-loja.ts`, issues 146/147) faz INSERT best-effort fire-and-forget em `admin_acessos` — tabela nova, RLS habilitada sem policy (deny-all para anon/authenticated, só `service_role`/BYPASSRLS lê/escreve — `schema.md` §2/§4), sem FK em `loja_id` (audit log sobrevive ao hard-delete da loja). `metadados` (jsonb) carrega só metadado operacional da própria ação (ex.: `{ modulo, ativo }`, `{ de, para }`) — nunca PII de comprador; `path` de Storage vai em `metadados`, não em `entidade_id` (coluna é `uuid`, path não é). Falha de INSERT (env ausente, rede, tabela indisponível) fica isolada num único try/catch e nunca propaga para a action de billing/permissão/PII que chamou (§14).
...
**`escopo.inserirVarios` — INSERT de N linhas em uma instrução (issue 269).** Mesma garantia do `inserir`, aplicada ao lote: `loja_id` injetado por último em CADA linha, via `svc.from(tabela).upsert(linhas.map(...), opcoes)` — `upsert`, não `insert`, porque o lote precisa ser tudo-ou-nada e `opcoes` (`onConflict`/`ignoreDuplicates`) só repassa idempotência, nunca escopo. Caminho sancionado para toda escrita admin de N linhas; `svc.from(tabela).upsert()` cru fora do wrapper é o mesmo risco cross-tenant do `insert` cru.
...
**Enforcement automático:** `src/app/admin/assinantes/enforcement-escopo-admin.test.ts` descobre os módulos de action via `readdirSync` (sem lista manual — action nova entra sozinha) e faz duas verificações estáticas de fonte por `export async function`: camada 2 exige referência a `prepararContextoAdmin`/`verificarAdminSaaS` (prova admin antes de qualquer efeito); camada 3 exige que todo statement `.from(tabela)....update()/.delete()/.insert()/.upsert()` cru contenha `.eq("loja_id", ...)` **nomeando a coluna de tenant** (issue 179 — a versão anterior aceitava qualquer `.eq(`, então um `update(...).eq("id", id)` cross-tenant passava). As escritas que escopam legitimamente por outra coluna vivem numa allowlist por (arquivo, tabela, verbo, coluna) com o motivo revisado — `bairros_zona` por `zona_id` (sem `loja_id` próprio, posse ancorada) e `lojas` por `id` (em `lojas` o `id` É a chave de tenant). A entrada da allowlist troca qual coluna é exigida, nunca dispensa o filtro. Uma action nova que esqueça o padrão falha no CI sem precisar editar a suíte de teste. A mesma descoberta cobre também os loaders `[lojaId]/carga*.ts` (`carga.ts`, `carga-opcionais.ts`, …) que elevam a `service_role` fora do fluxo de Server Action — o filtro é por prefixo (`carga` + `.ts`, excluindo `.test.ts`), não lista manual, então um loader novo que eleve privilégio entra automaticamente sob o mesmo guard (achado da issue 132: `carga-opcionais.ts` chegou a existir sem cair sob o enforcement até o glob ser generalizado).
...
**Camada 3 passou a casar `upsert` (issue 269, R1).** Até aqui a regex via só `update|delete|insert`; um `svc.from(tabela).upsert(...)` cru era invisível a todas as camadas — já havia um caso no repositório (`taxas_entrega` em `admin-entrega.ts`, legítimo por posse ancorada, mas passando por ausência de rede, não por aprovação). A issue 269 introduziu o segundo (`cardapio_produtos`, N linhas), então a rede fechou antes: o caminho sancionado é `escopo.inserirVarios` (acima), que injeta `loja_id` por último em cada linha. Para `upsert` a prova válida é a mesma de `insert` — posse ancorada revisada na allowlist, ou escopo por `loja_id` injetado pelo wrapper.

**"O verbo decide o instrumento" — duas provas de posse para escrita em `cardapio_produtos` (issues 270/274).** A issue 270 (achado da auditoria da 269) provou que `ON CONFLICT DO NOTHING` (`ignoreDuplicates`) descarta a linha alheia **antes** da FK composta rodar, e que um `DELETE` de zero linhas termina em sucesso mudo — nos dois casos o `count` não denuncia nada. A trava é `cardapioPertenceALoja(svc, lojaId, cardapioId)` (`lib/supabase/queries/cardapios.ts`), uma leitura ancorada **antes** de todo `upsert`/`inserirVarios`/`delete` em `cardapio_produtos` (`aplicarCardapioEmProdutos`, `tirarDeCardapio`, e as duas gêmeas admin). A issue 274 estendeu o padrão a `UPDATE`: `definirDiasDoVinculo`/`definirDiasDoVinculoAdmin` não chamam o gate porque um `UPDATE` com `count: "exact"` **transforma** "zero linhas" em recusa de verdade — a mesma garantia da 270, sem a segunda ida ao banco nem a janela TOCTOU entre ler e escrever. A segunda prova exige a chave **completa**: `loja_id` + `cardapio_id` + `produto_id` no `.eq` encadeado, com `count: "exact"` — ou, preferencialmente, `escopo.atualizarPorChave("cardapio_produtos", { cardapio_id, produto_id }, patch)` (novo método de `EscopoLoja`, abaixo), que produz essa tripla por construção. Chave parcial (só `cardapio_id`, alcançando o cardápio inteiro; só `produto_id`, alcançando o produto em todos os cardápios) ou `count` ausente (a recusa nunca dispara) são as duas formas letais.

**`EscopoLoja.atualizarPorChave` — UPDATE por chave natural, quando o cliente não conhece o `id` da linha (issue 274).** `atualizar` (acima) chaveia por `id`; `cardapio_produtos` é conhecido pelo par `(cardapio_id, produto_id)`, e resolver o `id` da junção por um SELECT prévio seria uma segunda ida ao banco e um oráculo de existência. `atualizarPorChave(tabela, chave, patch)` escopa por `loja_id` + **todas** as colunas de `chave`, com `count: "exact"` — a posse é provada **pela própria escrita**. Chave vazia **lança** (`atualizarPorChave: chave vazia`) em vez de devolver `{ ok: false }`: um UPDATE sem nenhuma coluna de chave degradaria para a loja inteira sob `service_role`, e isso é bug de programação, não caminho de usuário — fail-closed é a regra desta via. `patch` é tipado `Omit<Update, "loja_id" | "id" | K>`, mesma simetria de `inserir`/`atualizar` (barra por tipo re-parentear ou re-chavear a linha). **Débito conhecido (issue 282):** o guard de tipo é derrotável por cast (`as never`); falta o strip em runtime que `atualizarLoja` já tem — sem vetor a partir do cliente hoje (zod `.strict()` antes), prioridade baixa.

**Enforcement automático cruzando os dois mundos (issue 270, `cardapio-posse.enforcement.test.ts`).** Diferente do `enforcement-escopo-admin.test.ts` (só admin) e do `enforcement-escopo-queries.test.ts` (só leituras), esta suíte varre **todo** `src/**` por conteúdo (arquivo que menciona `cardapio_produtos`) e reprova, por `export async function`, qualquer `upsert`/`delete`/`inserirVarios` sem `cardapioPertenceALoja(` antes no mesmo corpo, e qualquer `update`/`atualizarPorChave` sem a tripla completa + `count: "exact"`. Cobre `src/lib/actions/cardapio.ts` (lojista, sob RLS) **e** `src/app/admin/assinantes/actions/admin-cardapios.ts` (admin, sob `service_role`) com o mesmo analisador — um caller novo que escreva em `cardapio_produtos` entra na descoberta sozinho, dos dois lados.

**Padrão admin para leituras (issue 130):** consultas de leitura sob `service_role` no hub admin usam a variante `(svc, lojaId)` das funções já existentes em `lib/supabase/queries/*.ts` — ex. `listarPedidosDaLoja`/`buscarPedidoDaLoja` (`pedidos.ts`), mesmo precedente de `buscarCategorias` (`categorias.ts`). Não passam pelo wrapper `EscopoLoja` (que cobre só escritas): o escopo é o `.eq("loja_id", lojaId)` explícito dentro da própria query, com `lojaId`/`id` validados por `schemaUuid` (`z.guid()`) antes de tocar o banco — formato inválido é fail-closed (`[]`/`null`, nunca vira query). `lojaId` é validado como UUID pelo caller (loader admin); a query repete o guard como defesa-em-profundidade, não como única validação. **Enforcement automático (issue 156 — fecha a lacuna que este parágrafo registrava):** `src/lib/supabase/queries/enforcement-escopo-queries.test.ts`, suíte irmã do guard admin, descobre por ASSINATURA toda função exportada `(svc, lojaId)` em `lib/supabase/queries/*` — a assinatura que marca "roda sob `service_role`, escopo é responsabilidade do código" — e exige `.eq("loja_id", lojaId)` no corpo. Uma query admin nova que esqueça o `.eq` entra na descoberta sozinha e falha no CI. Âncoras nominais nas funções atuais impedem que uma renomeação transforme o guard em no-op verde.
...
**Prop de Server Action obrigatória em componente compartilhado painel/admin (issue 160):** quando um `*Client.tsx` do painel é reusado por um wrapper `*AdminClient.tsx` (ex. `EntregasClient`/`EntregasAdminClient`, `ProdutosClient`/`CardapioAdminClient`), toda prop que carrega uma Server Action é **obrigatória no tipo, sem default apontando para a action do lojista**. Prop opcional com fallback do lojista compila mesmo quando o wrapper admin esquece de passá-la — e a action do lojista resolve a loja por `auth.uid()`, então o admin grava silenciosamente na própria loja em vez da loja-alvo da URL (cross-tenant silencioso). Dois casos reais fecharam por isso nesta issue: `alternarZonaAtiva` (`EntregasAdminClient`) e `reordenarCategorias` (`CardapioAdminClient`). **Enforcement automático:** `src/app/admin/assinantes/enforcement-props-action-admin.test.ts` descobre por filesystem todo `*AdminClient.tsx` sob `admin/assinantes/**`, parseia por AST do compilador TypeScript (não regex) e assere que cada wrapper injeta todas as props de action do client do painel que renderiza — cobre as duas formas de passagem (plana `onSalvar=` e aninhada `acoes={{...}}`), resolve alias importado de outro arquivo, `useMemo` e interseção de tipos, com sanity anti-vacuidade (≥ 9 wrappers descobertos, ≥ 1 prop por wrapper, ambas as formas populadas) para não virar no-op verde depois de um rename. Irmão de `enforcement-escopo-admin.test.ts` acima: aquele trava escopo por `loja_id` dentro da action; este trava que a action *certa* chega ao wrapper.
