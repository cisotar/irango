...
- Mecânica de browser que precisa ser testada sem jsdom (repo não usa jsdom) → módulo neutro (sem `'use client'`/`'use server'`) com o objeto global (janela, timer) **injetado por parâmetro**, nunca lido direto de `window`/`document` dentro da função — permite cobrir em `environment: node` com fake injetado. Padrão usado 4x: `criarControladorPolling`/`DepsPolling` (`confirmacao/StatusPedidoLive.tsx`, polling de status), `criarContagemAviso`/`DepsContagemAviso` (`confirmacao/avisoWhatsapp.ts`, issue 287 — storage e timer injetados; sucessor do `prepararAbaWhatsapp`/`AbrirJanela` de `checkout/aberturaWhatsapp.ts`, issue 126, removido na issue 287 porque o disparo do WhatsApp migrou do checkout para a confirmação), a medição da barra sticky da vitrine (`components/vitrine/medicaoBarraVitrine.ts`, issue 201 — elemento e `ResizeObserver` injetados) e o scrollspy da nav de categorias (`components/vitrine/scrollspyCategorias.ts`, issue 203 — `IntersectionObserver` e busca das `<section>` injetados)
- Operação assíncrona com **debounce + coalescência + revert** (ex.: salvar uma reordenação sem disparar uma request por toque, sem perder o último movimento, sem reverter para um estado que nunca existiu no banco quando respostas chegam fora de ordem) → máquina de estado pura em `lib/utils/`, a Server Action **injetada por parâmetro** (nunca importada dentro do módulo), usando só `setTimeout`/`clearTimeout` globais — o vitest troca por timers falsos, cobre em `environment: node` sem jsdom. `criarSalvamentoCoalescido` (`lib/utils/salvamento-coalescido.ts`, issue 175), reusado sem alteração pela reordenação de opcionais por categoria de produto (issue 209). Segunda instância do mesmo padrão, pelo mesmo motivo: `alternarAssociacaoOpcional` (`lib/utils/alternar-associacao-opcional.ts`, issue 213) extrai a orquestração do toggle de opcional porque a ORDEM das chamadas — flush do reorder pendente → nova seleção → gravação — é a trava de uma corrida (toggle disparado com movimento ainda no debounce mandaria à RPC uma sequência que não bate com as linhas persistidas; a RPC exige permutação completa e confere `row_count`, derrubando a transação), e dentro do componente era inalcançável por teste (`alternar()` só roda a partir de um `onCheckedChange`, que `renderToStaticMarkup` não dispara). Quem for escrever a próxima orquestração assíncrona hoje presa dentro de um componente sabe pela repetição do padrão: a saída é extrair
- **Lista arrastável do painel** (drag & drop + reordenar por teclado + salvamento coalescido) → miolo genérico em `components/painel/ModoReordenar.tsx` (`@dnd-kit` + `moverPorDeslocamento`/`mensagemPosicao` + `criarSalvamentoCoalescido`, handle imperativo `finalizar()` que o pai aguarda antes de desmontar/`router.refresh()`), extraído da issue 175 na issue 209 (`ReordenarCategorias.tsx` caiu de 393 para 112 linhas). Cada lista arrastável é uma casca fina sobre `ModoReordenar` que só sabe montar os itens e o formato do payload da própria Server Action: `ReordenarCategorias.tsx` (categorias de produto) e `ReordenarOpcionaisDaCategoria.tsx` (grupos de opcional dentro de uma categoria de produto, issue 209). **A próxima lista arrastável reusa `ModoReordenar`, não copia uma das cascas.** Desde a issue 213, `ModoReordenar` tem um segundo perfil de uso, **embutido** num cartão pai em vez de autônomo com card/status próprios: `semCartao`, `ocultarStatus`, `aoMudarStatus` e `arrastoBloqueado` (`/painel/produtos/opcionais` — a lista de grupos dentro do cartão de categoria de produto está sempre montada, sem botão "Reordenar", com status agregado no rodapé do cartão pai). Nota de manutenção, não débito: se aparecer um terceiro consumidor com mais variação, o corte melhor é agrupar essas quatro props num objeto `embutido?: {...}`.
