# [129] Allowlist: `CAMPOS_LOJA_SOMENTE_SERVIDOR` inclui os módulos (13→15)
...
## Objetivo
Impedir no código que qualquer patch (admin ou lojista) grave as flags de módulo.
Adicionar `modulo_impressao_a4` e `modulo_impressao_termica` à constante única
`CAMPOS_LOJA_SOMENTE_SERVIDOR` (`src/lib/actions/admin-loja.ts`), consumida pelo guard
de tipo `PatchLojaAdmin` e pelo filtro de runtime de `escopo.atualizarLoja`.

## Escopo
- [ ] `CAMPOS_LOJA_SOMENTE_SERVIDOR` (admin-loja.ts, linha 50): adicionar as duas colunas
  (13 → 15 entradas). O `satisfies readonly (keyof Tables["lojas"]["Update"])[]` garante
  que os nomes existam (quebra em build se renomearem).
- [ ] Confirmar que `escopo.atualizarLoja` já descarta as novas chaves no filtro de runtime
  (`Object.entries(...).filter(([k]) => !bloqueadas.includes(k))`) — sem código novo além
  da constante.
- [ ] Estender o teste de `atualizarLoja` (payload hostil com `modulo_impressao_*` é
  descartado antes do UPDATE).

## Fora de escopo
- Trigger de banco (issue 128 — camada complementar; esta é o filtro de código).
- Actions do lojista (`salvarPerfil`/`salvarHorarios`/`salvarTema`): montam patch explícito
  por allowlist de construção → as colunas de módulo já ficam de fora sem mudança.

## Reuso esperado
- `CAMPOS_LOJA_SOMENTE_SERVIDOR` — estender a constante existente, fonte única.
- Padrão de teste `admin-loja.binding.test.ts` / `admin-loja.test.ts` — reusar.

## Segurança
- **RN-M3 (filtro de código):** `svc` roda como `service_role` (BYPASSA o trigger 128) —
  este filtro é a autoridade quando o guard de tipo é derrotado por `as`/width-subtyping
  (ex.: admin-perfil casta para `TablesUpdate`). Sem esta entrada, um patch admin poderia
  ligar o módulo pago → burla de billing. Motivo da criticidade.
- Invariante preservada: as 13 colunas somente-servidor originais continuam bloqueadas.

## Critério de aceite
- [ ] (RED-first) Teste: `escopo.atualizarLoja({ ...perfil, modulo_impressao_a4: true } as
  never)` gera UPDATE **sem** `modulo_impressao_a4`/`modulo_impressao_termica` no patch.
- [ ] (RED-first) Teste: mesma proteção para `modulo_impressao_termica`.
- [ ] Vermelho escrito e confirmado ANTES do código; depois verde.
- [ ] Guard de tipo: `PatchLojaAdmin` não aceita as chaves de módulo (compila só sem elas).
- [ ] Suíte de `admin-loja` verde; `next build` passa.
