...
**Versão:** 0.2.27 | **Atualizado:** 2026-09-15
...
- Mecânica de browser que precisa ser testada sem jsdom (repo não usa jsdom) → módulo neutro (sem `'use client'`/`'use server'`) com o objeto global (janela, timer) **injetado por parâmetro**, nunca lido direto de `window`/`document` dentro da função — permite cobrir em `environment: node` com fake injetado. Padrão usado 3x: `criarControladorPolling`/`DepsPolling` (`confirmacao/StatusPedidoLive.tsx`, polling de status), `prepararAbaWhatsapp`/`AbrirJanela` (`checkout/aberturaWhatsapp.ts`, issue 126) e a medição da barra sticky da vitrine (`components/vitrine/medicaoBarraVitrine.ts`, issue 201 — elemento e `ResizeObserver` injetados)
