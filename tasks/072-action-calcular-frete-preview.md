...
## Escopo
- [ ] Criar Server Action `calcularFreteAction(loja_id: string, bairro: string)`
- [ ] Validar input com zod (`loja_id` uuid, `bairro` string min 1)
- [ ] Buscar zonas ativas tipo `bairro` + taxas + bairros via `lib/supabase/queries/`
- [ ] Buscar `taxa_entrega_fora_zona` (via `vitrine_lojas`)
- [ ] Reusar util de frete (070): match → `{ taxa_preview, zona_nome }`; fallback → `{ taxa_preview, zona_nome:'fora_zona' }`; nada → `{ zona_nome:'indisponivel' }`
- [ ] Retornar shape estável para o cliente
...
## Critério de aceite
- [ ] (crítica) Teste vermelho/verde: bairro em zona → `taxa_preview` da zona + `zona_nome`; bairro fora com fallback → `fora_zona` + taxa; loja sem zonas e sem fallback → `indisponivel`; preview usa exatamente a mesma lib do recálculo autoritativo.
