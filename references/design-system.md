...
- **Alvo de toque ≥ 44×44px** em todo elemento interativo da vitrine mobile (botões "Adicionar", controles de quantidade, "Finalizar pedido"). Mesma régua vale no painel quando o controle depende de gesto de toque (ex.: alça de arrasto e setas de reordenar categoria, `LinhaCategoriaReordenavel.tsx`, issues 159/175). Quando a linha ganha um prefixo antes da alça (prop `prefixo`, ex.: checkbox — issue 213), o chrome soma ~44px a mais e sobra pouco espaço pro nome em 360px; a régua nesse caso é **comprimir, não estourar 44px**: prop `compacta` (derivada de `prefixo != null`, nunca ligada por default) esconde as setas ↑↓ abaixo de `sm` e move os comandos para o kebab. É opt-in por consumidor — a linha sem prefixo (ex.: categorias de produto, issue 175) não precisa comprimir e não muda.
  - **Linha que ganha checkbox soma ~44px de chrome: comprima, não estoure** (issues 213/260). No modo de seleção de `/painel/produtos` (`ProdutosClient`, issue 260) a linha do produto recebe um `<label>` de `min-h-[44px] min-w-[44px]` antes da thumb. A saída registrada é a mesma de `LinhaCategoriaReordenavel`, aplicada por MODO em vez de por prop: enquanto o modo está ligado, o kebab, os chips de opcionais e os dois botões de estado **somem da linha** (não ficam `disabled`), porque no modo a única ação é a da barra. Em 360px sobram checkbox + thumb + nome/preço/badges, e nada estoura. Seleção de categoria inteira é **par de botões com o número escrito** (`Selecionar os 12` / `Limpar`), nunca checkbox tri-estado: o `Checkbox` gerado renderiza um `CheckIcon` fixo e um estado "mixed" exigiria editar arquivo do shadcn CLI.
  ...
  - **Sete pílulas numa linha só cabem cedendo largura, nunca altura (issues 275/276/277, `PilulasDeDias.tsx`).** No modo `compacto` (linha do vínculo produto↔cardápio e diálogo de lote) sete alvos de 44×44px + 6 gaps não cabem em 360px. Exceção registrada: a **altura** fica `min-h-[44px]` intacta; só o **eixo X** cede, com piso `min-w-[40px]` abaixo de `sm` (`sm:min-w-[44px]` a partir daí) — mesma régua "comprimir, não estourar" desta seção. O modo não-compacto (`FormVigencia`) não cede nada: `grid-cols-4 sm:grid-cols-7`, 44×44px literal nos dois eixos, como hoje.
